...
using Assets.LightShaft.Scripts.CustomTools;
using Assets.LightShaft.YoutubeAPI.Scripts.Src;
using Assets.LightShaft.YoutubeSrc;
using UnityEngine;
using UnityEngine.Events;
using UnityEngine.Networking;
using UnityEngine.UI;
using UnityEngine.Video;
using AudioType = Assets.LightShaft.YoutubeSrc.AudioType;
            ...
            else
            {
                ...
                foreach (VideoInfo info in videoInfos)
                {
                    if (info.VideoType == VideoType.Mp4 && info.Resolution == (quality))
                    {
                        ...
                        if (info.AudioType != AudioType.Unknown)//there's no audio atacched.
                        {
